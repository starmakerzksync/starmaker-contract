...
//        if (amount1In > 0) _update1(amount1In * PairFactory(factory).getFee(stable) / 10000); // accrue fees for token1 and move them out of pool
            if (amount0In > 0) _update0(amount0In * PairFactory(factory).getPairFee(address(this),stable) / 10000); // accrue fees for token0 and move them out of pool
            if (amount1In > 0) _update1(amount1In * PairFactory(factory).getPairFee(address(this),stable) / 10000); // accrue fees for token1 and move them out of pool
    ...
    function getAmountOut(uint amountIn, address tokenIn) external view returns (uint) {
        (uint _reserve0, uint _reserve1) = (reserve0, reserve1);
        amountIn -= amountIn * PairFactory(factory).getPairFee(address(this),stable) / 10000; // remove fee from amount received
